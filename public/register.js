...
class RegistrationSystem {
  ...
  init() {
    ...
    if (!this.form) {
      ...
      this.showCriticalError(
        "Form element not found. Please check your HTML structure."
      );
      return;
    }
    if (!this.firstnameInput) console.error("Firstname input not found");
    if (!this.lastnameInput) console.error("Lastname input not found");
  ...
  validateFirstname() {
    ...
    const formGroup = this.firstnameInput.closest(".form-group");
    const errorElement = document.getElementById("firstname-error");
    this.clearValidation(formGroup);

    if (!firstname) {
      this.showError(formGroup, errorElement, "First name is required");
      return false;
    }
  ...
  validateLastname() {
    ...
    const formGroup = this.lastnameInput.closest(".form-group");
    const errorElement = document.getElementById("lastname-error");
    this.clearValidation(formGroup);

    if (!lastname) {
      this.showError(formGroup, errorElement, "Last name is required");
      return false;
    }
  ...
  validateEmail() {
    ...
    if (!email.includes(".gov.ph")) {
      this.showWarning(
        formGroup,
        errorElement,
        "Consider using your official government email"
      );
    } else {
      this.showSuccess(formGroup);
    }
  ...
  focusNextInput(currentInput) {
    const inputs = Array.from(
      this.form.querySelectorAll('input[type="email"], input[type="password"]')
    );
