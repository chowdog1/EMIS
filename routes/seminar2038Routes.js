...
const Seminar2038 = require("../models/seminar2038");
...
router.get("/", async (req, res) => {
  try {
    const seminars = await Seminar2038.find().sort({ createdAt: -1 });
    res.json(seminars);
  } catch (error) {
    console.error("Error fetching 2038 seminars:", error);
  ...
  fs.createReadStream(req.file.path)
    ...
    .on("end", async () => {
      try {
        ...
        const savedSeminars = await Seminar2038.insertMany(results);
...
router.post("/send-invitations", verifyToken, async (req, res) => {
  try {
    ...
    const seminars = await Seminar2038.find({ status: "uploaded" });
...
router.post("/resend-invitation", verifyToken, async (req, res) => {
  try {
    ...
    const seminar = await Seminar2038.findOne({ email: email });
    if (!seminar) {
