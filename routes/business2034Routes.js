...
function transformToDatabaseFormat(data) {
  return {
    ...
    "2034_STATUS": data.status2034,
    "2034_NOTES": data.notes2034,
  };
}
...
// Get all businesses from 2034
router.get("/", async (req, res) => {
  try {
    const businesses = await Business2034.find({});
    res.json(businesses);
  } catch (error) {
    console.error("Error fetching 2034 businesses:", error);
    res.status(500).json({ message: "Server error" });
  }
});

// Get business statistics for 2034
router.get("/stats", async (req, res) => {
  try {
    const totalBusinesses = await Business2034.countDocuments();
    const highRiskCount = await Business2034.countDocuments({
      STATUS: "HIGHRISK",
    });
    const lowRiskCount = await Business2034.countDocuments({
      STATUS: "LOWRISK",
    });
    const renewalPendingCount = await Business2034.countDocuments({
      $or: [{ "AMOUNT PAID": { $exists: false } }, { "AMOUNT PAID": null }],
    });
    const activeBusinessesCount = await Business2034.countDocuments({
      "AMOUNT PAID": { $exists: true, $ne: null },
    });
    const renewalCount = await Business2034.countDocuments({
      "APPLICATION STATUS": "RENEWAL",
    });
    const newCount = await Business2034.countDocuments({
      "APPLICATION STATUS": "NEW",
    });
    const barangayStats = await Business2034.aggregate([
      { $group: { _id: "$BARANGAY", count: { $sum: 1 } } },
      { $sort: { count: -1 } },
    ]);
    const totalAmountResult = await Business2034.aggregate([
      { $group: { _id: null, totalAmount: { $sum: "$AMOUNT PAID" } } },
    ]);
    const totalAmountPaid =
      totalAmountResult.length > 0 ? totalAmountResult[0].totalAmount : 0;
    const monthlyTotals = await Business2034.aggregate([
      {
        $match: {
          "DATE OF PAYMENT": { $exists: true, $ne: null },
          $expr: { $eq: [{ $year: "$DATE OF PAYMENT" }, 2034] },
        },
      },
      {
        $group: {
    ...
    console.error("Error fetching 2034 business stats:", error);
    res.status(500).json({ message: "Server error" });
  }
});

// Search businesses in 2034
router.get("/search", async (req, res) => {
  try {
    ...
    switch (field) {
      case "accountNo":
        businesses = await Business2034.find({
    ...
    console.error("Error searching 2034 businesses:", error);
    res.status(500).json({ message: "Server error" });
  }
});

// Get a specific business from 2034
router.get("/account/:accountNo", async (req, res) => {
  try {
    const business = await Business2034.findOne({
    ...
    console.error("Error fetching 2034 business:", error);
    res.status(500).json({ message: "Server error" });
  }
});
...
router.get("/map", async (req, res) => {
  try {
    const businessesByBarangay = await Business2034.aggregate([
      {
        $group: {
          _id: "$BARANGAY",
          businesses: {
...
// Add a new business to 2034
router.post("/", verifyToken, async (req, res) => {
  try {
    console.log("Received data for 2034 business:", req.body);
    const processedBody = convertStringsToUppercase(req.body);
    const dbData = transformToDatabaseFormat(processedBody);
    const existingBusiness = await Business2034.findOne({
      "ACCOUNT NO": dbData["ACCOUNT NO"],
    });
    if (existingBusiness)
      return res.status(400).json({ message: "Account number already exists" });
    const newBusiness = new Business2034(dbData);
    const savedBusiness = await newBusiness.save();
    console.log("Business added to 2034:", savedBusiness);
    ...
    try {
      await syncBusinessToAllFutureYears("2034", savedBusiness.toObject());
    ...
    console.error("Error adding 2034 business:", error);
    res.status(500).json({ message: "Server error", details: error.message });
  }
});

// Update a business in 2034
router.put("/account/:accountNo", verifyToken, async (req, res) => {
  try {
    const currentBusiness = await Business2034.findOne({
      "ACCOUNT NO": req.params.accountNo,
    });
    if (!currentBusiness)
      return res.status(404).json({ message: "Business not found" });
    const processedBody = convertStringsToUppercase(req.body);
    const dbData = transformToDatabaseFormat(processedBody);
    const updatedBusiness = await Business2034.findOneAndUpdate(
      { "ACCOUNT NO": req.params.accountNo },
      dbData,
      { new: true },
    );
    ...
    // Sync to all future years
    try {
      await syncBusinessToAllFutureYears("2034", updatedBusiness.toObject());
    ...
    console.error("Error updating 2034 business:", error);
    res.status(500).json({ message: "Server error" });
  }
});

// Delete a business from 2034
router.delete("/account/:accountNo", verifyToken, async (req, res) => {
  try {
    const businessToDelete = await Business2034.findOne({
      "ACCOUNT NO": req.params.accountNo,
    });
    if (!businessToDelete)
      return res.status(404).json({ message: "Business not found" });
    await Business2034.deleteOne({ "ACCOUNT NO": req.params.accountNo });
    const accountNo = req.params.accountNo;
    await logAction(
      "DELETE",
      "business2034",
    ...
    try {
      await deleteBusinessFromAllFutureYears("2034", req.params.accountNo);
    ...
    console.error("Error deleting 2034 business:", error);
    res.status(500).json({ message: "Server error" });
  }
});
