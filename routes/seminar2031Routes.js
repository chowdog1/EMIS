...
// Get all seminars for 2031
router.get("/", async (req, res) => {
  try {
    const seminars = await Seminar2031.find().sort({ createdAt: -1 });
    res.json(seminars);
  } catch (error) {
  ...
  fs.createReadStream(req.file.path)
    ...
    .on("end", async () => {
      try {
        ...
        // Save to database
        const savedSeminars = await Seminar2031.insertMany(results);
...
router.post("/send-invitations", verifyToken, async (req, res) => {
  try {
    ...
    const seminars = await Seminar2031.find({ status: "uploaded" });
...
router.post("/resend-invitation", verifyToken, async (req, res) => {
  try {
    ...
    const seminar = await Seminar2031.findOne({ email: email });
    if (!seminar) {
