...
function transformToDatabaseFormat(data) {
  return {
    ...
    "2027_STATUS": data.status2027, // Map 2027 specific fields
    "2027_NOTES": data.notes2027, // Map 2027 specific fields
  };
}
...
// Get all businesses from 2027
router.get("/", async (req, res) => {
  try {
    const businesses = await Business2027.find({});
    res.json(businesses);
  } catch (error) {
    console.error("Error fetching 2027 businesses:", error);
    res.status(500).json({ message: "Server error" });
  }
});

// Get business statistics for 2027
router.get("/stats", async (req, res) => {
  try {
    // Total businesses count
    const totalBusinesses = await Business2027.countDocuments();
    // Count by status
    const highRiskCount = await Business2027.countDocuments({
      STATUS: "HIGHRISK",
    });
    const lowRiskCount = await Business2027.countDocuments({
      STATUS: "LOWRISK",
    });
    // Count businesses with no payments
    const renewalPendingCount = await Business2027.countDocuments({
      $or: [{ "AMOUNT PAID": { $exists: false } }, { "AMOUNT PAID": null }],
    });
    // Count active businesses
    const activeBusinessesCount = await Business2027.countDocuments({
      "AMOUNT PAID": { $exists: true, $ne: null },
    });
    // Count by application status
    const renewalCount = await Business2027.countDocuments({
      "APPLICATION STATUS": "RENEWAL",
    });
    const newCount = await Business2027.countDocuments({
      "APPLICATION STATUS": "NEW",
    });
    // Count by barangay
    const barangayStats = await Business2027.aggregate([
      { $group: { _id: "$BARANGAY", count: { $sum: 1 } } },
      { $sort: { count: -1 } },
    ]);
    // Calculate total amount paid
    const totalAmountResult = await Business2027.aggregate([
    ...
    const monthlyTotals = await Business2027.aggregate([
      {
        $match: {
          "DATE OF PAYMENT": { $exists: true, $ne: null },
          $expr: { $eq: [{ $year: "$DATE OF PAYMENT" }, 2027] },
        },
      },
      {
        $group: {
    ...
    console.error("Error fetching 2027 business stats:", error);
    res.status(500).json({ message: "Server error" });
  }
});

// Search businesses in 2027
router.get("/search", async (req, res) => {
  try {
    ...
    switch (field) {
      case "accountNo":
        businesses = await Business2027.find({
    ...
    console.error("Error searching 2027 businesses:", error);
    res.status(500).json({ message: "Server error" });
  }
});

// Get a specific business from 2027
router.get("/account/:accountNo", async (req, res) => {
  try {
    const business = await Business2027.findOne({
    ...
    console.error("Error fetching 2027 business:", error);
    res.status(500).json({ message: "Server error" });
  }
});
...
router.get("/map", async (req, res) => {
  try {
    ...
    const businessesByBarangay = await Business2027.aggregate([
      {
        $group: {
          _id: "$BARANGAY",
          businesses: {
...
// Add a new business to 2027
router.post("/", verifyToken, async (req, res) => {
  try {
    console.log("Received data for 2027 business:", req.body);
    ...
    const existingBusiness = await Business2027.findOne({
    ...
    const newBusiness = new Business2027(dbData);
    const savedBusiness = await newBusiness.save();
    console.log("Business added to 2027:", savedBusiness);
    ...
    // Sync to all future years (2028, 2029, 2030)
    try {
      await syncBusinessToAllFutureYears("2027", savedBusiness.toObject());
    ...
    console.error("Error adding 2027 business:", error);
    res.status(500).json({ message: "Server error", details: error.message });
  }
});

// Update a business in 2027
router.put("/account/:accountNo", verifyToken, async (req, res) => {
  try {
    // Get the current document before update
    const currentBusiness = await Business2027.findOne({
      "ACCOUNT NO": req.params.accountNo,
    });
    if (!currentBusiness) {
      return res.status(404).json({ message: "Business not found" });
    ...
    // Transform to database format
    const dbData = transformToDatabaseFormat(processedBody);
    const updatedBusiness = await Business2027.findOneAndUpdate(
      { "ACCOUNT NO": req.params.accountNo },
      dbData,
      { new: true }
    );
    ...
    // Sync to all future years (2028, 2029, 2030)
    try {
      await syncBusinessToAllFutureYears("2027", updatedBusiness.toObject());
    ...
    console.error("Error updating 2027 business:", error);
    res.status(500).json({ message: "Server error" });
  }
});

// Delete a business from 2027
router.delete("/account/:accountNo", verifyToken, async (req, res) => {
  try {
    // Get the document before deletion
    const businessToDelete = await Business2027.findOne({
      "ACCOUNT NO": req.params.accountNo,
    });
    if (!businessToDelete) {
      return res.status(404).json({ message: "Business not found" });
    }
    const result = await Business2027.deleteOne({
      "ACCOUNT NO": req.params.accountNo,
    });
    // Extract account number explicitly
    const accountNo = req.params.accountNo;
    ...
    // Delete from all future years (2028, 2029, 2030)
    try {
      await deleteBusinessFromAllFutureYears("2027", req.params.accountNo);
    ...
    console.error("Error deleting 2027 business:", error);
    res.status(500).json({ message: "Server error" });
  }
});
