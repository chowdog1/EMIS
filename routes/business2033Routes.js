...
function transformToDatabaseFormat(data) {
  return {
    ...
    "2033_STATUS": data.status2033,
    "2033_NOTES": data.notes2033,
  };
}
...
// Get all businesses from 2033
router.get("/", async (req, res) => {
  try {
    const businesses = await Business2033.find({});
    res.json(businesses);
  } catch (error) {
    console.error("Error fetching 2033 businesses:", error);
    res.status(500).json({ message: "Server error" });
  }
});

// Get business statistics for 2033
router.get("/stats", async (req, res) => {
  try {
    const totalBusinesses = await Business2033.countDocuments();
    const highRiskCount = await Business2033.countDocuments({
      STATUS: "HIGHRISK",
    });
    const lowRiskCount = await Business2033.countDocuments({
      STATUS: "LOWRISK",
    });
    const renewalPendingCount = await Business2033.countDocuments({
      $or: [{ "AMOUNT PAID": { $exists: false } }, { "AMOUNT PAID": null }],
    });
    const activeBusinessesCount = await Business2033.countDocuments({
      "AMOUNT PAID": { $exists: true, $ne: null },
    });
    const renewalCount = await Business2033.countDocuments({
      "APPLICATION STATUS": "RENEWAL",
    });
    const newCount = await Business2033.countDocuments({
      "APPLICATION STATUS": "NEW",
    });
    const barangayStats = await Business2033.aggregate([
      { $group: { _id: "$BARANGAY", count: { $sum: 1 } } },
      { $sort: { count: -1 } },
    ]);
    const totalAmountResult = await Business2033.aggregate([
      { $group: { _id: null, totalAmount: { $sum: "$AMOUNT PAID" } } },
    ]);
    const totalAmountPaid =
      totalAmountResult.length > 0 ? totalAmountResult[0].totalAmount : 0;
    const monthlyTotals = await Business2033.aggregate([
      {
        $match: {
          "DATE OF PAYMENT": { $exists: true, $ne: null },
          $expr: { $eq: [{ $year: "$DATE OF PAYMENT" }, 2033] },
        },
      },
      {
        $group: {
    ...
    console.error("Error fetching 2033 business stats:", error);
    res.status(500).json({ message: "Server error" });
  }
});

// Search businesses in 2033
router.get("/search", async (req, res) => {
  try {
    ...
    switch (field) {
      case "accountNo":
        businesses = await Business2033.find({
    ...
    console.error("Error searching 2033 businesses:", error);
    res.status(500).json({ message: "Server error" });
  }
});

// Get a specific business from 2033
router.get("/account/:accountNo", async (req, res) => {
  try {
    const business = await Business2033.findOne({
    ...
    console.error("Error fetching 2033 business:", error);
    res.status(500).json({ message: "Server error" });
  }
});
...
router.get("/map", async (req, res) => {
  try {
    const businessesByBarangay = await Business2033.aggregate([
      {
        $group: {
          _id: "$BARANGAY",
          businesses: {
...
// Add a new business to 2033
router.post("/", verifyToken, async (req, res) => {
  try {
    console.log("Received data for 2033 business:", req.body);
    const processedBody = convertStringsToUppercase(req.body);
    const dbData = transformToDatabaseFormat(processedBody);
    const existingBusiness = await Business2033.findOne({
      "ACCOUNT NO": dbData["ACCOUNT NO"],
    });
    if (existingBusiness)
      return res.status(400).json({ message: "Account number already exists" });
    const newBusiness = new Business2033(dbData);
    const savedBusiness = await newBusiness.save();
    console.log("Business added to 2033:", savedBusiness);
    ...
    try {
      await syncBusinessToAllFutureYears("2033", savedBusiness.toObject());
    ...
    console.error("Error adding 2033 business:", error);
    res.status(500).json({ message: "Server error", details: error.message });
  }
});

// Update a business in 2033
router.put("/account/:accountNo", verifyToken, async (req, res) => {
  try {
    const currentBusiness = await Business2033.findOne({
      "ACCOUNT NO": req.params.accountNo,
    });
    if (!currentBusiness)
      return res.status(404).json({ message: "Business not found" });
    const processedBody = convertStringsToUppercase(req.body);
    const dbData = transformToDatabaseFormat(processedBody);
    const updatedBusiness = await Business2033.findOneAndUpdate(
      { "ACCOUNT NO": req.params.accountNo },
      dbData,
      { new: true },
    );
    ...
    // Sync to all future years
    try {
      await syncBusinessToAllFutureYears("2033", updatedBusiness.toObject());
    ...
    console.error("Error updating 2033 business:", error);
    res.status(500).json({ message: "Server error" });
  }
});

// Delete a business from 2033
router.delete("/account/:accountNo", verifyToken, async (req, res) => {
  try {
    const businessToDelete = await Business2033.findOne({
      "ACCOUNT NO": req.params.accountNo,
    });
    if (!businessToDelete)
      return res.status(404).json({ message: "Business not found" });
    await Business2033.deleteOne({ "ACCOUNT NO": req.params.accountNo });
    const accountNo = req.params.accountNo;
    await logAction(
      "DELETE",
      "business2033",
    ...
    try {
      await deleteBusinessFromAllFutureYears("2033", req.params.accountNo);
    ...
    console.error("Error deleting 2033 business:", error);
    res.status(500).json({ message: "Server error" });
  }
});
