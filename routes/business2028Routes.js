...
function transformToDatabaseFormat(data) {
  return {
    ...
    "2028_STATUS": data.status2028, // Map 2028 specific fields
    "2028_NOTES": data.notes2028, // Map 2028 specific fields
  };
}
...
// Get all businesses from 2028
router.get("/", async (req, res) => {
  try {
    const businesses = await Business2028.find({});
    res.json(businesses);
  } catch (error) {
    console.error("Error fetching 2028 businesses:", error);
    res.status(500).json({ message: "Server error" });
  }
});

// Get business statistics for 2028
router.get("/stats", async (req, res) => {
  try {
    // Total businesses count
    const totalBusinesses = await Business2028.countDocuments();
    // Count by status
    const highRiskCount = await Business2028.countDocuments({
      STATUS: "HIGHRISK",
    });
    const lowRiskCount = await Business2028.countDocuments({
      STATUS: "LOWRISK",
    });
    // Count businesses with no payments
    const renewalPendingCount = await Business2028.countDocuments({
      $or: [{ "AMOUNT PAID": { $exists: false } }, { "AMOUNT PAID": null }],
    });
    // Count active businesses
    const activeBusinessesCount = await Business2028.countDocuments({
      "AMOUNT PAID": { $exists: true, $ne: null },
    });
    // Count by application status
    const renewalCount = await Business2028.countDocuments({
      "APPLICATION STATUS": "RENEWAL",
    });
    const newCount = await Business2028.countDocuments({
      "APPLICATION STATUS": "NEW",
    });
    // Count by barangay
    const barangayStats = await Business2028.aggregate([
      { $group: { _id: "$BARANGAY", count: { $sum: 1 } } },
      { $sort: { count: -1 } },
    ]);
    // Calculate total amount paid
    const totalAmountResult = await Business2028.aggregate([
    ...
    const monthlyTotals = await Business2028.aggregate([
      {
        $match: {
          "DATE OF PAYMENT": { $exists: true, $ne: null },
          $expr: { $eq: [{ $year: "$DATE OF PAYMENT" }, 2028] },
        },
      },
      {
        $group: {
    ...
    console.error("Error fetching 2028 business stats:", error);
    res.status(500).json({ message: "Server error" });
  }
});

// Search businesses in 2028
router.get("/search", async (req, res) => {
  try {
    ...
    switch (field) {
      case "accountNo":
        businesses = await Business2028.find({
    ...
    console.error("Error searching 2028 businesses:", error);
    res.status(500).json({ message: "Server error" });
  }
});

// Get a specific business from 2028
router.get("/account/:accountNo", async (req, res) => {
  try {
    const business = await Business2028.findOne({
    ...
    console.error("Error fetching 2028 business:", error);
    res.status(500).json({ message: "Server error" });
  }
});
...
router.get("/map", async (req, res) => {
  try {
    ...
    const businessesByBarangay = await Business2028.aggregate([
      {
        $group: {
          _id: "$BARANGAY",
          businesses: {
...
// Add a new business to 2028
router.post("/", verifyToken, async (req, res) => {
  try {
    console.log("Received data for 2028 business:", req.body);
    ...
    const existingBusiness = await Business2028.findOne({
    ...
    const newBusiness = new Business2028(dbData);
    const savedBusiness = await newBusiness.save();
    console.log("Business added to 2028:", savedBusiness);
    ...
    // Sync to all future years (2029, 2030)
    try {
      await syncBusinessToAllFutureYears("2028", savedBusiness.toObject());
    ...
    console.error("Error adding 2028 business:", error);
    res.status(500).json({ message: "Server error", details: error.message });
  }
});

// Update a business in 2028
router.put("/account/:accountNo", verifyToken, async (req, res) => {
  try {
    // Get the current document before update
    const currentBusiness = await Business2028.findOne({
      "ACCOUNT NO": req.params.accountNo,
    });
    if (!currentBusiness) {
      return res.status(404).json({ message: "Business not found" });
    ...
    // Transform to database format
    const dbData = transformToDatabaseFormat(processedBody);
    const updatedBusiness = await Business2028.findOneAndUpdate(
      { "ACCOUNT NO": req.params.accountNo },
      dbData,
      { new: true }
    );
    ...
    // Sync to all future years (2029, 2030)
    try {
      await syncBusinessToAllFutureYears("2028", updatedBusiness.toObject());
    ...
    console.error("Error updating 2028 business:", error);
    res.status(500).json({ message: "Server error" });
  }
});

// Delete a business from 2028
router.delete("/account/:accountNo", verifyToken, async (req, res) => {
  try {
    // Get the document before deletion
    const businessToDelete = await Business2028.findOne({
      "ACCOUNT NO": req.params.accountNo,
    });
    if (!businessToDelete) {
      return res.status(404).json({ message: "Business not found" });
    }
    const result = await Business2028.deleteOne({
      "ACCOUNT NO": req.params.accountNo,
    });
    // Extract account number explicitly
    const accountNo = req.params.accountNo;
    ...
    // Delete from all future years (2029, 2030)
    try {
      await deleteBusinessFromAllFutureYears("2028", req.params.accountNo);
    ...
    console.error("Error deleting 2028 business:", error);
    res.status(500).json({ message: "Server error" });
  }
});
