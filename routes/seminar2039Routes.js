...
router.get("/", async (req, res) => {
  try {
    const seminars = await Seminar2039.find().sort({ createdAt: -1 });
    res.json(seminars);
  } catch (error) {
  ...
  fs.createReadStream(req.file.path)
    ...
    .on("end", async () => {
      try {
        ...
        const savedSeminars = await Seminar2039.insertMany(results);
...
router.post("/send-invitations", verifyToken, async (req, res) => {
  try {
    ...
    const seminars = await Seminar2039.find({ status: "uploaded" });
...
router.post("/resend-invitation", verifyToken, async (req, res) => {
  try {
    ...
    const seminar = await Seminar2039.findOne({ email: email });
    if (!seminar) {
